...
void bellman_ford(vertex *new_vertex, int number_of_vertices, vector<Pair> graph[]) {
    if (new_vertex != NULL) {
        ...
        for (int i = 0; i < number_of_vertices - 1; i++) {
            for (int j = 0; j <= number_of_vertices; j++) {
                for (int k = 0; k < graph[j].size(); k++) {
                    ...
                    if (new_vertex->distance[j] != INT_MAX && new_vertex->distance[j] + w < new_vertex->distance[v]) {
                        new_vertex->distance[v] = new_vertex->distance[j] + w;
                        new_vertex->previous[v] = j;
                    }
                }
            }
        }
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i <= number_of_vertices; ++i) {
        bellman_ford(&vertices[i], number_of_vertices, graph);
    }
